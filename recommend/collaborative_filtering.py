from .models import Alcohol, History
...
def collaborative_filtering():
    ...
    with open(BASE_DIR + "/static/dataset_cf.csv", 'w', encoding='utf-8', newline='') as csv_file:
    ...
    alco = pandas.read_csv(BASE_DIR + "/static/alcohol_cf.csv", encoding='utf-8')
    alco = alco.set_index('alco_name')

    data = pandas.read_csv(BASE_DIR + "/static/dataset_cf.csv", encoding='utf-8').fillna(0)
    ...
    data.to_csv(BASE_DIR + "/static/dataset_cf.score", sep=' ', header=None, index=False, encoding='utf-8')

    reader = Reader(line_format='user item rating', sep=' ')
    dataset = Dataset.load_from_file(BASE_DIR + "/static/dataset_cf.score", reader=reader)
    ...
    with open(BASE_DIR + "/static/answer_cf.csv", 'w', encoding='utf-8', newline='') as csv_file:
